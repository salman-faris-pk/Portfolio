  ...
  return (
    <div className="md:hidden top-5 right-5 fixed w-60 z-[999] flex flex-col items-end gap-2">
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            className="w-full bg-white drop-shadow border border-slate-400 dark:border-white border-opacity-40 shadow-2xl rounded-2xl flex flex-col items-center justify-center dark:bg-gray-950 p-1"
            variants={menuList}
            initial="start"
            animate="visible"
          ...
          >
            {links.map((link, index) => (
              ...
              >
                <NextLink
                  className={clsx(
                    "flex w-full items-center justify-center px-3 py-3 hover:text-gray-950 transition dark:hover:text-gray-300 cursor-pointer",
                    {
                      "text-gray-950 bg-slate-200 dark:text-gray-200 dark:bg-gray-700 rounded":
                        activeSection === link.hash,
                      "rounded-t-xl rounded": index === 0,
                      "rounded-b-xl rounded": index === links.length - 1,
                    }
                  )}
                  href={link.hash}
                  onClick={() => {
